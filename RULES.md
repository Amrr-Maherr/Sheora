...
Any developer working on this project MUST read and follow this file before implementing or modifying any feature.
...
Use **axios** for all HTTP requests through the centralized API client.

> Rule: All HTTP calls MUST go through axios. Do not use `fetch` for API requests.
...
Feature API
   ↓
API Client (axios, services/api/client.ts)
   ↓
Backend
```
...
Use **shadcn/ui** for all reusable UI primitives.

> Rule: Any new UI primitive MUST come from shadcn/ui (via the CLI) — do not hand-roll buttons, inputs,
> dialogs, etc. when an appropriate shadcn/ui component exists.

shadcn/ui components live in `components/ui` and are composed from `@/components`.
...
- `type` over `interface` for ALL type declarations — object shapes, props, and generics MUST be written with `type`
- type aliases
- generics
- inferred types where appropriate

> Rule: Always declare types with `type`, never `interface`. Do not mix both in the same file.
...
- write complex code when a simple version exists

> Rule: Keep the code as simple as possible at all times. If a simple solution works,
> use it. Complexity is only justified when it is strictly required by the feature.
